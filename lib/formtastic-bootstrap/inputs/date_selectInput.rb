  ...
  module Inputs
    class DateSelectInput < Formtastic::Inputs::DateSelectInput
      ...
      def fragment_html(fragment)
        template.content_tag(:span, :class => FRAGMENT_CLASSES[fragment]) do
          fragment_input(fragment)
        end
      end

      def fragment_input(fragment)
        template.select_tag(
          fragment_name(fragment),
          fragment_options(fragment),
      ...
      def bootstrap_wrapping(&block)
        template.content_tag(:div, :class => 'form-group') do
          label_html << template.content_tag(:div, :class => 'col-sm-10') do
            yield
          end
        end
      end
